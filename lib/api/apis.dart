...
class APIs {
  ...
  static Future<void> updatePhotoUrlGalleryM(BuildContext context) async {
    try {
  ...
  static Future<void> updatePhotoUrlCamera(BuildContext context) async {
    try {
      LoadingScreen().show(context: context, text: 'Please Wait...');
  ...
  static Future<void> deleteProfilePicM(BuildContext context) async {
    try {
      if (me.image == "") return;
      ...
      if (me.image.startsWith(fireStoregeUrl)) {
      ...
      LoadingScreen().hide();
      Dialogs.showSnackbar(context, 'Something went wrong');
      log('form delete photo mobile hash = ${e.hashCode}, runtimeType = ${e.runtimeType}');
    }
  }
    ...
    try {
      ...
      var res = await post(
        ...
        headers: {
          ...
          HttpHeaders.authorizationHeader: 'key=$auth',
        },
      );
